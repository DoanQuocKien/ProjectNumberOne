"""
Shared builder for Final_PIR_Jan_Submission.ipynb and Final_PIR_Dec_Submission.ipynb.

Memory architecture (fixes OOM):
- ALL expensive side tables (SVD embeddings, bestsellers by loc/cat/brand,
  ui_hist, archetypes, events, I2I similarity) are precomputed ONCE and are small.
- Candidate generation runs per-chunk using these lookup tables.
- No single DataFrame ever exceeds CHUNK_SIZE * ~500 rows.
- del + gc.collect() after every major intermediate.
- Float32 throughout.
- Completely Transaction-Only (no event file required!).
...
def precompute_lookup_tables(hist_tx, items, all_users):
    ...
    # Subsample users for lightning-fast SVD during Optuna tuning
    np.random.seed(42)
    if len(u_map) > 15000:
        svd_users = np.random.choice(u_map, size=15000, replace=False)
    else:
    ...
    print("  [LUT] Fitting SVD on float32 matrix...", flush=True)
    # Using arpack solver because the randomized solver causes thread deadlocks in Kaggle OpenBLAS
...
def assemble_dataset(candidates, target_tx, tables, is_inference=False):
    ...
    i_map_df   = pl.DataFrame({"item_id": i_arr.tolist(),
                                "i_idx":   np.arange(len(i_arr), dtype=np.int32)}, schema={"item_id": pl.Utf8, "i_idx": pl.Int32})
    u_map_pl   = pl.DataFrame({"customer_id": list(u_map_local.keys()),
                                "u_local_idx": list(u_map_local.values())}, schema={"customer_id": pl.Int64, "u_local_idx": pl.Int32}).with_columns(
                                    pl.col("u_local_idx").cast(pl.Int32))
...
best_params = {
    ...
    'learning_rate': 0.011038373942922366, 'num_leaves': 55, 'min_data_in_leaf': 336,
    'feature_fraction': 0.8252733687573659, 'bagging_fraction': 0.602863341255194, 'bagging_freq': 4,
    'lambda_l1': 0.09237718274475376, 'lambda_l2': 0.130751986081969,
    'random_state': 42, 'n_jobs': -1, 'verbose': -1, 'feature_pre_filter': False
}
"""


def make_tuning_notebook():
    cell_data = """
items = load_items()
"""
    ...
    cell_optuna = """
# ==============================================================
# OPTUNA TUNING (Train: Nov, Valid: Dec)
# ==============================================================
import optuna
...
# Stream larger chunks directly into LightGBM to stay OOM safe on Kaggle
lgb_train = build_lgb_dataset_streaming(targ_nov, tables_nov, FEATURES, sample_n=15000)
...
lgb_valid = build_lgb_dataset_streaming(targ_dec, tables_dec, FEATURES, sample_n=5000)
...
"""
    ...
    nb["cells"] = [
        ...
        nbf.v4.new_code_cell("!pip install optuna"),
        nbf.v4.new_code_cell(CELL_HELPERS),
        nbf.v4.new_code_cell(cell_data),
        nbf.v4.new_code_cell(cell_optuna)
    ]
    with open("Tuning_Optuna.ipynb", "w", encoding="utf-8") as f:
        nbf.write(nb, f)
    print("Tuning_Optuna.ipynb created.")


if __name__ == "__main__":
    make_tuning_notebook()
