...
print("\n--- f1 Target stats ---")
lbls1 = f1['target'].to_list()
u1, c1 = np.unique(lbls1, return_counts=True)
print(dict(zip(u1.tolist(), c1.tolist())))
...
g3 = f3.group_by('customer_id', maintain_order=True).len()['len'].to_numpy()
print(f"Group sizes in f3: {len(g3)} groups, max size={int(g3.max())}, min size={int(g3.min())}")

cust_ids = f3['customer_id'].to_numpy()
targets = f3['target'].to_numpy()

idx = 0
homogeneous_count = 0
for sz in g3:
    g_cust = cust_ids[idx : idx + sz]
    g_targ = targets[idx : idx + sz]
    
    assert np.all(g_cust == g_cust[0]), f"Scramble detected inside group at index {idx}!"
    
    if len(np.unique(g_targ)) == 1:
        homogeneous_count += 1
    idx += sz

print(f"Total groups: {len(g3)}")
print(f"Homogeneous groups (either all 0s or all 1s): {homogeneous_count} ({homogeneous_count/len(g3)*100:.2f}%)")
