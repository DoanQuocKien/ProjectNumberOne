...
print("--- [CELL 1] Loading and Mapping Data ---")
...
# Read a larger slice to guarantee months 3, 4, 5 exist
df_raw = pl.read_parquet(T_PATH, n_rows=400000).join(item_mapping, on='item_id', how='inner').drop('item_id').rename({'item_int_id': 'item_id'}).select([
...
def standardize_age(text):
    raw_text = str(text).strip().lower()
    if re.search(r'(\\*|x\\d|cm)', raw_text): return 0.5
    if re.search(r'\\bb\\d{2}\\b', raw_text): return 18.0
    if 's17' in raw_text: return 1.0
    if '110' in raw_text: return 5.0
    if "không xác định" in raw_text or not raw_text: return -1.0
    diaper_map = {
    ...
    for pattern, val in diaper_map.items():
        if re.search(pattern, raw_text): return val
    range_match = re.search(r'(\\d+\\.?\\d*)\\s*-\\s*(\\d+\\.?\\d*)', raw_text)
    if range_match:
        s, e = float(range_match.group(1)), float(range_match.group(2))
        avg = (s + e) / 2
        if any(x in raw_text for x in ['m', 'tháng']): return round(avg / 12, 3)
        return avg
    m_match = re.search(r'(\\d+\\.?\\d*)\\s*(m|tháng)', raw_text)
    if m_match: return round(float(m_match.group(1)) / 12, 3)
    y_match = re.search(r'(\\d+\\.?\\d*)\\s*(y|t|tuổi)', raw_text)
    if y_match: return float(y_match.group(1))
    pure_num = re.search(r'^(\\d+)$', raw_text)
    if pure_num:
...
# Cell 4 simulation
print("--- [CELL 4] Simulating Training Folds & LightGBM ---")
h1 = df_raw.filter(pl.col('month') <= 3)
t1 = df_raw.filter(pl.col('month') == 4)
...
# Generate a small training set to train a tiny LGBM
X = f1[all_feats].to_numpy()
y = f1['target'].to_numpy()
g = f1.group_by('customer_id', maintain_order=True).len()['len'].to_numpy()

if len(X) == 0:
    print("Fold is empty under slice, creating dummy data to run LGBM training...")
    X = np.random.rand(100, len(all_feats))
    y = np.random.choice([0, 1], 100)
    g = np.repeat(10, 10)

dtrain = lgb.Dataset(X, y, group=g, feature_name=all_feats)
lgb_m = lgb.train(
    {'objective': 'lambdarank', 'metric': 'ndcg', 'ndcg_eval_at': [10], 'verbosity': -1, 'random_state': SEED},
    dtrain,
    num_boost_round=10
)
...
# Cell 5 simulation
print("--- [CELL 5] Simulating Chunked Submission Loop ---")
h_full = df_raw.filter(pl.col('month') <= 12)
target_users = h_full['customer_id'].unique().head(1000).to_list() # small test slice of target users

def make_chunked_submission_pkl(history_df, items_df, target_users, model, all_feats, output_path, chunk_size=500):
    with open("idx2item.pkl", "rb") as f_map:
        idx2item = pickle.load(f_map)
    retriever = V12Retriever(history_df, items_df)
    
    print(f"Beginning submission generation for {len(target_users)} users in chunks...")
    
    with open(output_path, 'wb') as f_out:
        for i in range(0, len(target_users), chunk_size):
            chunk_u = target_users[i:i+chunk_size]
            cands = retriever.get_candidates(chunk_u)
            
            if cands.is_empty():
                continue
                
            # Simulate feature building inside the loop (polars lazy)
            cands_feat = cands.join(items_df.select(['item_id', 'item_age_proxy'] + [f'{c}_id' for c in cat_cols]), on='item_id', how='left')
            cands_feat = cands_feat.with_columns([
                pl.col('item_id').cast(pl.Int32),
                pl.col('item_age_proxy').fill_null(0.0).cast(pl.Float32)
            ])
            for c in cat_cols:
                cands_feat = cands_feat.with_columns(pl.col(f"{c}_id").fill_null(0).cast(pl.Int32))
                
            # Fill remaining features with 0 to match all_feats
            missing_feats = [feat for feat in all_feats if feat not in cands_feat.columns]
            if missing_feats:
                cands_feat = cands_feat.with_columns([pl.lit(0.0).alias(feat) for feat in missing_feats])
                
            # Rank predictions
            X_chunk = cands_feat[all_feats].to_numpy()
            preds = model.predict(X_chunk)
            
            cands_scored = cands.with_columns(pl.Series("score", preds))
            
            # Extract top 10 recommended items
            top_recs = cands_scored.sort(['customer_id', 'score'], descending=[False, True])\
                                   .group_by('customer_id')\
                                   .head(10)\
                                   .group_by('customer_id')\
                                   .agg(pl.col('item_id').alias('recs'))
                                   
            for row in top_recs.iter_rows():
                cust = row[0]
                items = row[1]
                str_items = [idx2item[item] for item in items]
                pickle.dump((cust, str_items), f_out)
                
    print(f"Submission streaming completed! Recommendations saved to {output_path}")

make_chunked_submission_pkl(h_full, items_df, target_users, lgb_m, all_feats, "submission.pkl", chunk_size=500)

print("\n" + "="*60)
print("ALL CELLS OF THE PIPELINE EXECUTED SUCCESSFULLY AND END-TO-END!")
print("="*60 + "\n")
