...
# Recency profile: sort by event_ts descending and select unique items per user
user_recency = hist.filter(pl.col('customer_id').is_in(test_users))\
    .sort('event_ts', descending=True)\
    .unique(subset=['customer_id', 'item_id'], keep='first')\
    .group_by('customer_id')\
    .agg(pl.col('item_id'))

recs_dict = {row[0]: list(row[1]) for row in user_recency.iter_rows()}
...
for uid in test_users:
    ...
    # Recommend top recency items, pad with global popularity if < 10
...
print(f"Recency-based Baseline: Precision@10 = {prec:.6f}, Hits = {hits}, Users = {total_users}")
