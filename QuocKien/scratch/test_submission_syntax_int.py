...
df_raw = pl.read_parquet(T_PATH, n_rows=10000).join(item_mapping, on='item_id', how='left').drop('item_id').rename({'item_int_id': 'item_id'}).select([
        ...
        if u_idx:
            ...
            for i in range(0, len(u_idx), chunk):
                ...
                c_svd.append(pl.DataFrame({
                    ...
                    'item_id': pl.Series(i_arr[t60.flatten()], dtype=pl.Int32) # Explicit Int32 cast
                }))
                ...
                c_i2i.append(pl.DataFrame({
                    ...
                    'item_id': pl.Series(i_arr[t80.flatten()][mask.flatten()], dtype=pl.Int32) # Explicit Int32 cast
                }))
...
def test_submission_pkl(history_df, items_df, target_users, model, all_feats, output_path, chunk_size=20):
    ...
    for idx in range(0, len(target_users), chunk_size):
        ...
        for customer_id, items in zip(c_ids, item_lists):
            # Stream the ORIGINAL String item IDs back into pickle!
        ...
        print("Chunk processed successfully with Int32 mapping!")
        break

test_submission_pkl(df_raw, items_df, target_users, model, all_feats, Path('test_sub.pkl'))
print("RIGOROUS INT32 AND STRING CACHE TEST COMPLETED SUCCESSFULLY!")
