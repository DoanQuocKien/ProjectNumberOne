...
def check_fold_details(train_end, val_m):
    ...
    np.random.seed(SEED)
    sampled_users = list(np.random.choice(all_users.to_list(), min(60000, len(all_users)), replace=False))
    ...
    f = create_dataset_v12(h, t, items_df, sample_users=60000, n_negatives=150)
    return f

print("Generating 60,000 user f1 and f3 folds...")
f1 = check_fold_details(8, 9)
f3 = check_fold_details(10, 11)

print("\n--- f3 Target Stats ---")
lbls3 = f3['target'].to_list()
u3, c3 = np.unique(lbls3, return_counts=True)
print(dict(zip(u3.tolist(), c3.tolist())))
...
print(f"y3 length: {len(y3)}")
print(f"g3 sum: {g3.sum()}")
print(f"Number of groups in g3: {len(g3)}")
...
for sz in g3:
    ...
    if not np.all(g_cust == g_cust[0]):
        print(f"SCRAMBLE DETECTED AT GROUP INDEX {idx}!")
    
    g_targ = y3[idx : idx + sz]
    if len(np.unique(g_targ)) == 1:
        homogeneous_count += 1
    idx += sz

print(f"Homogeneous ratio: {homogeneous_count / len(g3) * 100:.2f}%")

print("\nTraining LightGBM Lambdarank to verify score...")
param = {
    'objective': 'lambdarank', 'metric': 'ndcg', 'ndcg_eval_at': [10], 'verbosity': -1,
    'learning_rate': 0.013958417519338458,
    'num_leaves': 360,
    'max_depth': 17,
    'min_data_in_leaf': 524,
    'lambda_l1': 0.011602777845814163,
    'lambda_l2': 5.346083998797776,
    'max_bin': 255, 'random_state': SEED
}
dtrain = lgb.Dataset(X1, y1, group=g1, feature_name=all_feats, categorical_feature=cat_feat_ids)
